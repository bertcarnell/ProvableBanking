...
x <- c(1/12, 3/12, 6/12, c(1,2,3,5,7,10,20,30))
x_labels <- c("1mo", "3mo", "6mo", 1, 2, 3, 5, 7, 10, 20, 30)
...
g1 <- ggplot(df1, aes(x = x, y = y, group = date)) + 
  ...
  scale_x_log10(limits = c(0.08, 31), breaks = x, labels = x_labels, name = "Years") +
  ggtitle("Yield Curve Changes in 2018")
plot(g1)
...
* All loans and deposit products are paid or charged interest at the end of the term.  The
entire principle and interest are paid back at the end of the term

```{r}
df1 <- data.frame(x = x,
                  y = c(1.5, 1.6, 1.7, 1.85, 2.0, 2.2, 2.5, 3.0, 3.5, 4.0, 4.5))
...
* Interest Income = `r scales::dollar(0.03*200000)`
* Interest Expense = `r scales::dollar(0.0050*100000)`
* Net Interest Income = Spread Income = `r scales::dollar(0.03*200000-0.0050*100000)`
* Return on Average Assets = `r scales::percent((0.03*200000-0.0050*100000)/200000)`
* Return on Equity = `r scales::percent((0.03*200000-0.0050*100000)/100000)`
...
* Interest Income = `r scales::dollar(0.0315*200000)`
* Interest Expense = `r scales::dollar(0.0035*100000)`
* Net Interest Income = Spread Income = `r scales::dollar(0.0315*200000-0.0035*100000)`
* Return on Average Assets = `r scales::percent((0.0315*200000-0.0035*100000)/200000)`
* Return on Equity = `r scales::percent((0.0315*200000-0.0035*100000)/100000)`
