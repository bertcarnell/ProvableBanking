...
- Investment 1 is a 1 year bond where you are paid the principal and interest
...
at the end of the year, and then the principal is returned in the last year.
...
npv1 <- (-start) * (1 + inflation_rate) ^ 30 + ret1
...
f <- function(rate2)
{
  (sum(rep(rate2 * start, 30)*(1 + inflation_rate) ^ (1:30) + c(rep(0, times = 29), start) + 
         c(-start * (1 + inflation_rate) ^ 30, rep(0, times = 29))) - npv1)^2
}
o_inf <- optimize(f = f, interval = c(0, 1))

```

- Investment 1 returns \$1.81 at the end of the 30 years, for a total of \$0.81 in return
and \$0.34 in NPV
- Investment 2 returns \$1.81 at the end of the 30 years summing the coupon payments at 
an interest rate of 2.7%
- Investment 2 returns $0.34 in NPV at an interest rate of 2.3%
